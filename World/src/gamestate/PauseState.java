...
public class PauseState extends GameState{
	...
	public void placeButtons(){
		...
		buttons.add(new GameButton(
				...
				(GamePanel.HEIGHT / 3) + (height / 2) + BUTTONSPACE, width, height, text));
		
		text = "MENU";
		width = fm.stringWidth(text);
		buttons.add(new GameButton(
				(GamePanel.WIDTH / 2) - (width / 2), 
				(GamePanel.HEIGHT / 3) + (height / 2) + BUTTONSPACE * 2, width, height, text));
	}
	...
	public void draw(Graphics2D g) {
		...
		int width = fm.stringWidth("PAUSE");
		int height = fm.getHeight();
		g.drawString("PAUSE", (GamePanel.WIDTH / 2) - (width / 2), (GamePanel.HEIGHT / 3) + (height / 2));
